...
if __name__ == "__main__":
    ...
    VOCdevkit_path = sys.argv[5]
    save_dir = sys.argv[6]
    miou_out_path = sys.argv[7]
    cls_weights_list = sys.argv[8]
    #---------------------------------#
    #   Cuda    是否使用Cuda
    #           没有GPU可以设置成False
    #---------------------------------#
    ...
    #   num_classes = 3
    cls_weights=[]
    for each in cls_weights_list.split(','):
        cls_weights.append(int(each))
    
    cls_weights = np.array(cls_weights, np.float32)
    #cls_weights = np.array([1,2,2,2,2,2], np.float32)
    ...
    if model_path != '':
        ...
        print(list(model_dict)[:20])
        pretrained_dict = torch.load(model_path, map_location = device)
        load_key, no_load_key, temp_dict = [], [], {}
        print(list(pretrained_dict)[:20])
    ...
    if True:
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, VOCdevkit_path, log_dir,Cuda,miou_out_path,eval_flag=eval_flag, period=eval_period)
        else:
            eval_callback   = None
